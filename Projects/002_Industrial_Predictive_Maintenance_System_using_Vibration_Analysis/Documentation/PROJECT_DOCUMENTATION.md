...
> **Note:** The sketch uses m/s² (acceleration) rather than mm/s (velocity). The standard conversion for a sinusoidal signal is: `velocity (m/s) = acceleration (m/s²) / (2π × frequency_Hz)`, then multiply by 1000 for mm/s. Since the current version does not perform frequency analysis (no FFT), use the m/s² thresholds directly and adjust them by baseline testing. FFT-based frequency classification is listed in Future Enhancements.
